...
public class CommsCallback implements Runnable {
		...
		if (token != null) {
			...
			if (asyncCB != null) {
				if (token.getException() == null) {
					...
					//log.fine(className, methodName, "716",new Object[] { token.internalTok.getKey() });
	...
	private void handleMessage(MqttPublish publishMessage)
		...
		if (mqttCallback != null) {
			...
			//log.fine(className, methodName, "713", new Object[] { new Integer(publishMessage.getMessageId()), destName });
